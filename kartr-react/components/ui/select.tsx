...
export function ReusableSelect<T extends FieldValues>({
  ...
  return (
    ...
    >
      <SelectTrigger
        className={cn(
          value ? '' : 'text-secondary-foreground/20',
          'bg-card border-secondary-foreground/20'
        )}
      >
        <SelectValue
      ...
      <SelectContent className="bg-card border-secondary-foreground/20 cursor-pointer">
        {groups
          ? groups.map((group) => (
              <SelectGroup key={group.label} className="">
                <SelectLabel className="">{group.label}</SelectLabel>
